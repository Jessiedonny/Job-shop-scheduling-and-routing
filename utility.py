...
def calculate_euclidean_distance(px, py, index1, index2):
    ...
    dist=round(math.sqrt((px[index1]-px[index2])**2+(py[index1]-py[index2])**2))
...
def calculate_total_distance(routes, px, py, depot):
    ...
    for i in range(len(routes)):
        dist=calculate_euclidean_distance(px,py,depot,routes[i][0])  #calculate the distance from depot to the first node
        for j in range(len(routes[i])-1):  #make sure the last node is captured by j+1
            index1=routes[i][j]
            index2=routes[i][j+1]
            dist+=calculate_euclidean_distance(px,py,index1,index2)
        dist+=calculate_euclidean_distance(px,py,routes[i][len(routes[i])-1],depot)
        total_dist+=dist
    return total_dist
